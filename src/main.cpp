...
unsigned int nStakeTargetSpacing = 2 * 60; // 1-minute block spacing
int64 nChainStartTime = 1396983139;
...
const string strMessageMagic = "Eliza Signed Message:\n";
...
int64 GetProofOfStakeReward(int64 nCoinAge, unsigned int nBits, unsigned int nTime)
{
    ...
    if(fTestNet || nTime > PROTOCOL_SWITCH_TIME)
    {
        ...
        // Eliza: reward for coin-year is cut in half every 64x multiply of PoS difficulty
        // A reasonably continuous curve is used to avoid shock to market
        ...
        CBigNum bnLowerBound = 100 * CENT; // Lower interest bound is 1% per year
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    bool fEnforceBIP30 = true; // Always active in Eliza
    bool fStrictPayToScriptHash = true; // Always active in Eliza
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "Eliza", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // Genesis block
        const char* pszTimestamp = "April 8th, 2014 3:02pm EST. Bitcoin akin to the Internet 20 years ago, digital currency executive says";
        CTransaction txNew;
...
unsigned char pchMessageStart[4] = { 0xe3, 0xf7, 0xf3, 0xc5 };
